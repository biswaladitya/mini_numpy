#include "compute.h"


// Computes the dot product of vec1 and vec2, both of size n
int dot(uint32_t n, int32_t *vec1, int32_t *vec2) {
 int32_t sum = 0;
 for (int i = 0; i < n; i++) {
   sum += vec1[i] * vec2[i];
 }
 return sum;
}


// Computes the convolution of two matrices
int convolve(matrix_t *a_matrix, matrix_t *b_matrix, matrix_t **output_matrix) {
 // Allocate memory for the output matrix
 *output_matrix = (matrix_t *) malloc(sizeof(matrix_t));
 (*output_matrix)->rows = a_matrix->rows - b_matrix->rows + 1;
 (*output_matrix)->cols = a_matrix->cols - b_matrix->cols + 1;
 (*output_matrix)->data = (int32_t *) malloc((*output_matrix)->rows * (*output_matrix)->cols * sizeof(int32_t));

 int i;
 int j;
 int temp; 

 //flip matrix B
 
 i = 0; 
 j = (b_matrix->rows * b_matrix->cols) - 1; 

 while (i < j) { 
  temp = b_matrix->data[j]; 
  b_matrix->data[j] = b_matrix->data[i]; 
  b_matrix->data[i] = temp; 
  i++; 
  j--;
 }

 // Perform the convolution operation

  for (i = 0; i < (*output_matrix)->rows; i++) {
   for (j = 0; j < (*output_matrix)->cols; j++) {
    int32_t sum = 0;

    for (int k = 0; k < b_matrix->rows; k++) {
      int leftIndex = (i+k) * (a_matrix->cols) + j; 
      int rightIndex = leftIndex + b_matrix->cols; 
      sum += dot(b_matrix->cols, a_matrix->data + leftIndex, b_matrix->data + (k * b_matrix->cols)); 
    }
    (*output_matrix)->data[i * (*output_matrix)->cols + j] = sum;
   }
  }
  
  return 0; 
}


// Executes a task
int execute_task(task_t *task) {
 matrix_t *a_matrix, *b_matrix, *output_matrix;


 if (read_matrix(get_a_matrix_path(task), &a_matrix))
   return -1;
 if (read_matrix(get_b_matrix_path(task), &b_matrix))
   return -1;


 if (convolve(a_matrix, b_matrix, &output_matrix))
   return -1;


 if (write_matrix(get_output_matrix_path(task), output_matrix))
   return -1;

 free(a_matrix->data);
 free(b_matrix->data);
 free(output_matrix->data);
 free(a_matrix);
 free(b_matrix);
 free(output_matrix);
 return 0;
}



